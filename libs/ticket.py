...
class Ticket:
    ...
    def from_nomor(cls, noticket: str):
        ...
        if (
            not res.ok
            or "Tiket Tidak Ditemukan, silakan Lakukan Pencarian Ulang" in res.text
        ):  # NOQA
            return from_dict(cls, data)
        ...
        td = table.findAll("td")
        # data,
        status = soup.find("div", class_=["col-md-4", "col-sm-4"]).find("span").text
        data["status"] = status
        try:
            data["nama"] = th[2].text
            ...
            if status == "CLOSE":
                ...
                data[
                    "warning"
                ] = f"status = {status} tidak dikenali, mohon hubugi @hexatester untuk mengimplementisakannya."  # NOQA
    ...
    def reply_markup(self):
        ...
        if self.status == "OPEN":
            keyboard[0].append(
                InlineKeyboardButton("Refresh", callback_data=self.callback_data)
            )
    ...
    def inline_query_article(self):
        return InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(message_text=self.string),
